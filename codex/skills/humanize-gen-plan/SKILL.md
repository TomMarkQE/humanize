...
Generate the requested Goal Plan without implementing repository changes. The current Codex thread owns requirements, decisions, synthesis, validation, and the final write. A native `humanize_researcher` child may own bounded behaviorally no-write repository investigation. Effective child permissions inherit from the live parent task, so the parent verifies the repository state before integrating research.
...
Before spawning a researcher, record the current branch, exact `HEAD`, staged/unstaged tracked-file status, and untracked non-Humanize files. Preserve any pre-existing dirty state exactly. The parent may continue reasoning, reading, and preparing in-memory plan content while the child runs, but must not change repository state until the child has been collected and the baseline verified.

The child prompt must state that no file, index, branch, commit, or untracked non-Humanize state may be created or changed. This is a mandatory behavioral contract, not a claim of a separate hard sandbox.
...
- explicit no-write and non-implementation boundaries;
...
3. Prepare the plan skeleton and initial AC/task candidates in memory based only on known inputs.
4. Identify exactly which skeleton fields depend on delegated evidence.

Do not call the wait tool immediately unless no useful parent work remains. Do not repeat the repository investigation or write the final plan while the child is active.

## Integration gate

Collect the child before making the final relevance decision, freezing affected scope, finalizing acceptance criteria, or writing the plan. Before reading its result into the plan, compare the current branch, `HEAD`, index, tracked-file status, and untracked non-Humanize set against the recorded baseline.

If any repository state changed, treat the child as having violated its no-write role. Do not integrate the result, do not restore or discard changes automatically, identify the exact delta, and report `agent_failed` or `permission_denied` as appropriate. When the baseline matches, verify critical citations directly and mark unsupported claims as uncertainty rather than converting them into requirements.
...
Resolve cross-references, dependency IDs, AC mappings, language consistency, and internal contradictions before writing. Write the final plan only after all required child evidence has been collected, baseline-verified, and integrated. Report the output path, AC count, delegated research status, and unresolved decisions.
